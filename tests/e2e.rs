...
pub mod e2e {
    ...
    #[test]
    fn print_error_when_api_key_missing() -> Result<(), Box<dyn std::error::Error>> {
        ...
        // check output
        cmd.assert()
            .stderr(predicates::str::contains("Could not find an API key"))
    ...
    #[test]
    fn store_api_key() -> Result<(), Box<dyn std::error::Error>> {
        // empty directory
        let mock_fs = FsMockBuilder::new()?;
        let mock_data_dir = mock_fs.path();
        ...
        cmd.arg("--local-dir").arg(mock_data_dir);
        cmd.arg("--sync-url").arg(server_url);
        cmd.arg("--set-api-key").arg("MOCK_API_KEY");

        // check output
        cmd.assert()
            .stdout(predicates::str::contains("Stored API key"));
        ...
        assert_eq!(file_contents["api_key"], toml::Value::from("MOCK_API_KEY"));
        Ok(())
    }
